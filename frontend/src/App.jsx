import { ArrowLeft, Boxes, Loader2, LogOut, ShieldCheck, Users } from "lucide-react";
...
const highlights = [
  ...
  },
  {
    ...
    text: "פרטי קשר חוזרים רק מהשרת ורק לפי חוקי ההרשאה של הקהילה."
  }
];

function App() {
  const { user, demoCommunity, isLoading, logout } = useAuth();
  ...
  if (user) {
    return <AuthenticatedHome user={user} demoCommunity={demoCommunity} onLogout={logout} />;
  }
...
function AuthenticatedHome({ user, demoCommunity, onLogout }) {
  return (
    <main className="min-h-screen bg-stone-50 text-slate-950">
      ...
      <section className="mx-auto max-w-6xl px-5 py-10">
        <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm font-semibold text-slate-500">התחברת בהצלחה</p>
          <h2 className="mt-2 text-3xl font-bold">בסיס האימות מוכן</h2>
          <p className="mt-3 max-w-2xl leading-8 text-slate-700">
            בשלב הבא נוסיף יצירת קהילות, הצטרפות עם קוד, ואישור חברים על ידי מנהל קהילה.
          </p>

          {demoCommunity ? (
            <div className="mt-6 rounded-md bg-teal-50 p-4 text-teal-950">
              <p className="font-bold">קהילת דמו פעילה: {demoCommunity.name}</p>
              <p className="mt-1 text-sm">קוד הצטרפות: {demoCommunity.joinCode}</p>
            </div>
          ) : null}
        </div>
      </section>
